  ...
  async execute(interaction) {
    try {
      ...
      if (!interaction.guild) {
		...
		// Find roles for tank and cloth

	  const tankRole = interaction.guild.roles.cache.find((role) => role.name === 'tank');
const clothRole = interaction.guild.roles.cache.find((role) => role.name === 'cloth');
  // Get the command content
      const commandContent = interaction.content.slice('!cloth'.length).trim();
...
const channelName = `cloth-${type}-${gold}-${authorUsername}`;
 ...
 allow: [PermissionsBitField.Flags.ViewChannel],
   ...
   // Allow tank role to view the channel
    ...
    // Allow cloth role to view the channel
    {
      id: clothRole.id,
      allow: [PermissionsBitField.Flags.ViewChannel],
    },
      ...
      console.log('New channel ID:', newChannel.id);
	     ...
	     // Mention the cloth and tank role in the message content


// Convert role mentions to string
const tankRoleMention = tankRole?.toString();
const clothRoleMention = clothRole?.toString();
    console.log('Bot mention all (Tank):', tankRoleMention,'And cloth', clothRoleMention);
...
.addFields(
  ...
  // content: `${dpsRoleMention} A new channel has been created!`,
    ...
    await newChannel.send({
     content: `${tankRoleMention} and ${clothRoleMention} A new channel has been created!`,
     //   embeds: [embed],
  ...
  embed: embed,
  clothRoleMention: clothRoleMention,
    tankRoleMention: tankRoleMention,
};
// Assuming clothRole and tankRole are already defined Role objects from your guild
const clothMention = clothRole.toString(); // Converts the cloth role to a mention string
