  ...
  async execute(interaction) {
    try {
      ...
      if (!interaction.guild) {
		...
		// Find roles for tank and nostack

	  const tankRole = interaction.guild.roles.cache.find((role) => role.name === 'tank');
const healRole = interaction.guild.roles.cache.find((role) => role.name === 'heal');
const dpsRole = interaction.guild.roles.cache.find((role) => role.name === 'DPS');
  // Get the command content
      const commandContent = interaction.content.slice('!nostack'.length).trim();
...
const channelName = `nostack-${type}-${gold}-${authorUsername}`;
 ...
 allow: [PermissionsBitField.Flags.ViewChannel],
   ...
   // Allow tank role to view the channel
    ...
    // Allow heal role to view the channel
    {
      id: healRole.id,
      allow: [PermissionsBitField.Flags.ViewChannel],
    },
	 // Allow DPS role to view the channel
    {
      id: dpsRole.id,
      allow: [PermissionsBitField.Flags.ViewChannel],
    },
      ...
      console.log('New channel ID:', newChannel.id);
	     ...
	     // Mention the nostack and tank role in the message content


// Convert role mentions to string
const tankRoleMention = tankRole?.toString();
const healMention = healRole?.toString();
const dpsRoleMention = dpsRole?.toString();
console.log('Bot mention all (Tank):', tankRoleMention, 'Heal:', healMention, 'DPS:', dpsRoleMention);
...
.addFields(
  ...
  // content: `${dpsRoleMention} A new channel has been created!`,
    ...
    await newChannel.send({
 content: `${tankRoleMention}, ${healMention}, ${dpsRoleMention} A new channel has been created!`,
     //   embeds: [embed],
...
const postData = {
    ...
    healMention: healRole.id,
    tankRoleMention: tankRole.id,
	dpsRoleMention: dpsRole.id,
};
