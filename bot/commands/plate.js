  ...
  async execute(interaction) {
    try {
      ...
      if (!interaction.guild) {
		...
		// Find roles for tank and plate

	
const plateRole = interaction.guild.roles.cache.find((role) => role.name === 'plate');
  // Get the command content
      const commandContent = interaction.content.slice('!plate'.length).trim();
...
const channelName = `plate-${type}-${gold}-${authorUsername}`;
 ...
 allow: [PermissionsBitField.Flags.ViewChannel],
    ...
    // Allow plate role to view the channel
    {
      id: plateRole.id,
      allow: [PermissionsBitField.Flags.ViewChannel],
    },
      ...
      console.log('New channel ID:', newChannel.id);
	     ...
	     // Mention the plate and tank role in the message content


// Convert role mentions to string

const plateRoleMention = plateRole?.toString();
    console.log('Bot mention all plate:', plateRoleMention);
...
content: `${plateRoleMention} A new channel has been created!`,
  ...
  embed: embed,
  plateRoleMention: plateRoleMention,
 
};
// Assuming plateRole and tankRole are already defined Role objects from your guild
const plateMention = plateRole.toString(); // Converts the plate role to a mention string
