  ...
  async execute(interaction) {
    try {
      ...
      if (!interaction.guild) {
		...
		// Find roles for tank and mail

	  const tankRole = interaction.guild.roles.cache.find((role) => role.name === 'tank');
const mailRole = interaction.guild.roles.cache.find((role) => role.name === 'mail');
  // Get the command content
      const commandContent = interaction.content.slice('!mail'.length).trim();
...
const channelName = `mail-${type}-${gold}-${authorUsername}`;
 ...
 allow: [PermissionsBitField.Flags.ViewChannel],
   ...
   // Allow tank role to view the channel
    {
      id: tankRole.id,
      allow: [PermissionsBitField.Flags.ViewChannel],
    },
    // Allow mail role to view the channel
    {
      id: mailRole.id,
      allow: [PermissionsBitField.Flags.ViewChannel],
    },
      ...
      console.log('New channel ID:', newChannel.id);
	     ...
	     // Mention the mail and tank role in the message content


// Convert role mentions to string
const tankRoleMention = tankRole?.toString();
const mailRoleMention = mailRole?.toString();
    console.log('Bot mention all (Tank):', tankRoleMention,'And Mail', mailRoleMention);
...
.addFields(
  ...
  // content: `${dpsRoleMention} A new channel has been created!`,
    ...
    await newChannel.send({
     content: `${tankRoleMention} and ${mailRoleMention} A new channel has been created!`,
     //   embeds: [embed],
  ...
  embed: embed,
  mailRoleMention: mailRoleMention,
    tankRoleMention: tankRoleMention,
};
// Assuming mailRole and tankRole are already defined Role objects from your guild
const mailMention = mailRole.toString(); // Converts the mail role to a mention string
const tankMention = tankRole.toString(); // Converts the tank role to a mention string
...
const postData = {
    ...
    mailRoleMention: mailRole.id,
    tankRoleMention: tankRole.id,
};


// Send the POST request to the PHP server
axios.post('http://152.228.154.27/wowbot/commandexe/channelmake.php', postData)
    .then(response => {
...
await fetch('http://152.228.154.27/wowbot/commandexe/checkdata.php', {
  method: 'POST',
  headers: {
